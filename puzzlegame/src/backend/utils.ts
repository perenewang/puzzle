...
export function formatTime(time: number) {
    ...
    let result = `${formatNumber(Math.floor(absTime / 6e4 % 60), 2)
        }:${formatNumber(Math.floor(absTime / 1e3 % 60), 2)
        }`;
    if (time < 36e5) return (time < 0 ? '-' : '') + result;
    result = `${formatNumber(Math.floor(absTime / 36e5 % 24), 2)
        }:${result}`;
    if (time < 864e5) return (time < 0 ? '-' : '') + result;
    return `${formatNumber(Math.floor(time / 864e5), 2)
        }d ${result}`;
}
